...
import { wallet, abi, contractaddress,coinaddress, coinabi } from "../store.js";
...
async function mint(number, referrer){
  ...
  let estimate = await contract.estimateGas.mint(number, referrer , {value: spend});
...
async function mintAndPurge(number, referrer){
  const cost = await contract.cost();
  const spend = BigInt(cost * number)
  let estimate = await contract.estimateGas.mintAndPurge(number, referrer , {value: spend});
  estimate = BigInt(parseInt(estimate * 1.05))
  await contract.mintAndPurge(number, referrer , {value: spend, gasLimit:estimate});
}
  ...
  <div class="relative h-full mx-2">
    ...
    <p class="mt-10">
      <button @click="mintButton(mint)" class="p-2 bg-black border rounded">
        ...
        <button @click="mintButton(mintAndPurge)" class="p-2 bg-black border rounded">
        Mint And Purge
      </button>
    ...
    <p>
      <button @click="coinMintButton(coinMint)" class="p-2 bg-black border rounded">
        Coin Mint
      </button>
        <button @click="coinMintButton(coinMintAndPurge)" class="p-2 bg-black border rounded">
        Coin Mint And Purge
      </button>
    </p>
    <p>
